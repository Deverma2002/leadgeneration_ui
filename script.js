...
/* ── GA4 CLIENT ID COOKIE EXTRACTOR ────────────────────────── */
function getGaClientId() {
  try {
    ...
    if (match) {
      ...
      if (parts.length >= 4) {
        return `${parts[2]}.${parts[3]}`; // Extracts '1698360958.1786440929'
      }
    }
  } catch (e) {
    console.warn('[Analytics] Could not read _ga cookie:', e);
  }
  return null;
}
...
async function submitLead(e) {
  ...
  const payload = {
    ...
    requirement: byId('requirement').value.trim(),
    source:      'Nexora Landing',
    gaClientId:  getGaClientId(), // Dynamic Google Analytics Client ID
    tracking: {
      pid:        q.get('pid')        || null,
      oid:        q.get('oid')        || null,
      clid:       q.get('clid')       || null,
